...
import { axiosPublic, axiosPrivate } from 'config/axiosInstance';
...
useEffect(() => {
    ...
    const result = await axiosPrivate.get(`/profile/edit/${_id}`);
        try {
            console.log(result);
    ...
    >
      {(props) => (
        <Form onSubmit={props.handleSubmit}>
                <h3 className='register__header'> Edit Profile </h3>
                ...
                <Field
                  as='textarea'
                  name='bio'
                  placeholder='What makes you different? Tell people about yourself in less than 250 charaters.'
                  className='register__emailField'
                />
                {props.touched.lastName && <p className='register__errorMessage'> {props.errors.lastName} </p>}
                </label>
                ...
                <Button type='submit' className='register__button--register'> { props.isSubmitting ? <SyncLoader color='white' size={8} /> : 'Save' } </Button>
        </Form>
      )}
    </Formik>
    </div>
